...
+ (id)showAction:(NSString *)title message:(NSString *)message items:(NSArray<NSString *> *)items onCancelBlock:(void (^)(void))cancelBlock onItemBlock:(void (^)(NSInteger itemIndex))itemBlock{
    ...
    if(!DD_StringIsEmpty(message)){
        ...
        CGFloat alertMessageHeight = [vctl.alertMessageAttr
                                      boundingRectWithSize:CGSizeMake((DD_ScreenWidth-vctl.alertMarginLeft-vctl.alertMarginRight-DDSimpleActionMarginLeftRight*2), CGFLOAT_MAX)
...
- (void)setupSubviews{
    ...
    [self.contentView layerCorners:(UIRectCornerBottomLeft|UIRectCornerBottomRight) cornerRect:CGRectMake(0, 0, CGRectGetWidth([UIScreen mainScreen].bounds)-self.alertMarginLeft-self.alertMarginRight, self.contentHeight) cornerSize:self.cornerSize];
    [self.cancelButton layerCorners:(UIRectCornerAllCorners) cornerRect:CGRectMake(0, 0, CGRectGetWidth([UIScreen mainScreen].bounds)-self.alertMarginLeft-self.alertMarginRight, self.bottomHeight-20) cornerSize:self.cornerSize];
}
