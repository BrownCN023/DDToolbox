//
//  DDSimpleConfirmViewController.m
//  DDModalDemo
//
...
@interface DDSimpleConfirmViewController ()
...
@implementation DDSimpleConfirmViewController

+ (id)showAlert:(NSString *)title
        message:(NSString *)message
  onCancelBlock:(void (^)(void))cancelBlock
 onConfirmBlock:(void (^)(void))confirmBlock{
    return [self showAlert:title message:message cancel:nil confirm:nil onCancelBlock:cancelBlock onConfirmBlock:confirmBlock];
}
...
+ (id)showAlert:(NSString *)title
  ...
  onCancelBlock:(void (^)(void))cancelBlock
 onConfirmBlock:(void (^)(void))confirmBlock{
    DDSimpleConfirmViewController * vctl = [[[self class] alloc] init];
    ...
    if(!DDModal_StringIsEmpty(message)){
        ...
        CGFloat alertMessageHeight = [vctl.alertMessageAttr
                                      boundingRectWithSize:CGSizeMake((DDModal_ScreenWidth-vctl.popMarginLeft-vctl.popMarginRight-DDConfirmAlertMarginLeftRight*2), CGFLOAT_MAX)
