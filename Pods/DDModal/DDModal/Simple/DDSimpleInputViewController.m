...
onValidationBlock:(BOOL (^)(NSString * text))validationBlock{
    return [self showAlert:title text:text placeholder:placeholder cancel:nil confirm:nil onCancelBlock:cancelBlock onSubmitBlock:submitBlock onValidationBlock:validationBlock];
}

+ (id)showAlert:(NSString *)title text:(NSString *)text placeholder:(NSString *)placeholder cancel:(NSString *)cancel confirm:(NSString *)confirm onCancelBlock:(void (^)(void))cancelBlock onSubmitBlock:(void (^)(NSString * text))submitBlock onValidationBlock:(BOOL (^)(NSString * text))validationBlock{
    DDSimpleInputViewController * vctl = [[[self class] alloc] init];
    vctl.cancelTitle = cancel;
    vctl.confirmTitle = confirm;
...
- (DDSimpleInputAlertTextField *)textField{
    if(!_textField){
        ...
        v.backgroundColor = DDModal_COLOR_Hex(0xF6F6F9);
...
- (UIButton *)okButton{
    if(!_okButton){
        UIButton * v = [UIButton buttonWithType:UIButtonTypeSystem];
        v.frame = CGRectMake(0, 0, 100, 40);
        [v setTitle:(self.confirmTitle?self.confirmTitle:@"确定") forState:UIControlStateNormal];
        //        v.backgroundColor = DD_COLOR_Random();
